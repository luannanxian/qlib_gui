...
class TestImportTaskRepository:
    ...
    async def test_get_by_user_with_status_filter(
        ...
        """Test getting import tasks by user ID with status filter"""
        # Arrange
        # Create two tasks with different statuses for the same user
        pending_task_data = {
        ...
        pending_task = await import_task_repo.create(obj_in=pending_task_data, commit=True)
        completed_task = await import_task_repo.create(obj_in=completed_task_data, commit=True)

        # Act - Filter by PENDING status
        pending_tasks = await import_task_repo.get_by_user(
            "filter_user_123",
        ...
        assert all(t.status == ImportStatus.PENDING.value for t in pending_tasks)
        assert all(t.user_id == "filter_user_123" for t in pending_tasks)
        # Verify completed task is not in the results
        assert not any(t.id == completed_task.id for t in pending_tasks)

        # Cleanup
        await import_task_repo.delete(pending_task.id, soft=False, commit=True)
        await import_task_repo.delete(completed_task.id, soft=False, commit=True)

    @pytest.mark.asyncio
    async def test_get_by_dataset(
        self,
        import_task_repo: ImportTaskRepository
    ):
        """Test getting import tasks by dataset ID"""
        # Arrange
        dataset_id = "test_dataset_abc123"
        task_data = {
            "task_name": "Dataset Import Task",
            "import_type": ImportType.CSV.value,
            "status": ImportStatus.COMPLETED.value,
            "original_filename": "dataset_import.csv",
            "file_path": "/tmp/dataset_import.csv",
            "file_size": 3072,
        ...
        # Act
        dataset_tasks = await import_task_repo.get_by_dataset(dataset_id)

        # Assert
        assert len(dataset_tasks) >= 1
        assert any(t.id == task.id for t in dataset_tasks)
        assert all(t.dataset_id == dataset_id for t in dataset_tasks)
